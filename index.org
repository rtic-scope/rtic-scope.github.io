...
- Frontend(s) :: While tracing and replaying, a trace can be forwarded to a set of frontends via Unix domain sockets where virtually endless analytics can be applied.
  ...
  At present, information only flows from the backend to the frontend dummy.
  In the future, bidirectional communication will be possible, enabling a frontend
  to implement a complex hardware-in-the-loop testing suite.
  Please refer to the [[Roadmap][project roadmap]] for future prospects.
- The target-side tracing crate :: ~cortex-m-rtic-trace~, is a small auxiliary crate applied to the target application when tracing.
...
Below is a list of the main repositories that constitute the RTIC project.
...
- [[https://github.com/rtic-scope/rfcs][rfcs]] :: A catch-all meta-repository for discussions and feature suggestions that encompass more than a single repository.
...
- A CMSIS-DAP hardware probe supported by ~probe-rs~ /or/ some serial hardware that exposes a serial device which then can be used to read the trace stream from the target device.
...
* Concepts
- Source :: a [trace] **source** is any implementation of [[https://github.com/rtic-scope/cargo-rtic-scope/blob/master/src/sources/mod.rs#L20][=trait Source=]] from which decoded trace packets can be pulled via an iterator using for example [[https://doc.rust-lang.org/std/iter/trait.Iterator.html#tymethod.next][=Iterator::next=]].
...
Software tasks are similarly traced, but come at a cost of a write to a =u32= variable when entering and exiting the task, resulting
in a cost of two writes.
...
2. The RTIC application declaration, =#[app(...)] mod app {...}=, is parsed from ~blinky~'s source code.
   ...
   This parsing step places some restrictions on how the source code for an RTIC application can be written. Refer to [[RTIC application constraints]].
...
- Where are all build artifacts stored? :: Most likely under ~target/~, assuming the current working directory is a crate containing target applications.
  The target directory can be overridden via the enviornment variable =TARGET_DIR= or the =--target-dir= option.
...
* License
For non-commercial purposes, RTIC Scope is licensed under both the MIT License and the Apache License (Version 2.0).
